...
function mousePressed() {
  if (soundsLoaded) {
    if (collected && !dispersing) {
      ...
      disperseSound.play(); // Play dispersal sound
      particles.forEach((p) => {
        p.arrived = false; // Reset arrival flag for future interactions
        p.stuckToCursor = false; // Release any particles stuck to the cursor
      });
      dispersing = true; // Switch to dispersal mode
    } else if (dispersing) {
      // Return to forming the word on the second click
      formSound.play(); // Play forming sound
      particles.forEach((p) => {
        p.arrived = false; // Reset arrival flag
        p.stuckToCursor = false; // Release cursor-stuck particles
      });
